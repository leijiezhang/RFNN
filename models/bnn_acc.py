...
from models.model_run_acc import bnn_cmp_mthds
import torch
import os
import scipy.io as io


def run_bnn(param_config: ParamConfig):
    """
    todo: run the codes for bnn
    :param param_config: parameter configurations
    :return:
    """
    # Dataset configuration
    param_config.log.info(f"this model is a bnn test model")
    param_config.log.info(f"inference method: {param_config.inference}")
    ...
    param_config.log.info(f"n_samples : {param_config.n_samples}")

    if param_config.inference == "nuts":
        param_config.log.info(f"step_size : {param_config.step_size}")
        param_config.log.info(f"num_steps : {param_config.num_steps}")
        param_config.log.info(f"warmup : {param_config.warmup}")
    elif param_config.inference == "svi":
        param_config.log.info(f"svi_lr  : {param_config.lr_svi}")
        param_config.log.info(f"svi epoch number : {param_config.n_epoch_svi}")
    ...
    for i in torch.arange(len(param_config.dataset_list)):
        ...
        # mlp saving tensors
        mlp121_train_acc_tsr = torch.empty(1, 0).to(param_config.device)
        mlp121_test_acc_tsr = torch.empty(1, 0).to(param_config.device)
        mlp421_train_acc_tsr = torch.empty(1, 0).to(param_config.device)
        mlp421_test_acc_tsr = torch.empty(1, 0).to(param_config.device)
        mlp21_train_acc_tsr = torch.empty(1, 0).to(param_config.device)
        mlp21_test_acc_tsr = torch.empty(1, 0).to(param_config.device)
        mlp12421_train_acc_tsr = torch.empty(1, 0).to(param_config.device)
        mlp12421_test_acc_tsr = torch.empty(1, 0).to(param_config.device)
        mlp212_train_acc_tsr = torch.empty(1, 0).to(param_config.device)
        mlp212_test_acc_tsr = torch.empty(1, 0).to(param_config.device)
        mlp42124_train_acc_tsr = torch.empty(1, 0).to(param_config.device)
        mlp42124_test_acc_tsr = torch.empty(1, 0).to(param_config.device)

        # 1d-cnn saving tensors
        cnn11_train_acc_tsr = torch.empty(1, 0).to(param_config.device)
        cnn11_test_acc_tsr = torch.empty(1, 0).to(param_config.device)
        cnn12_train_acc_tsr = torch.empty(1, 0).to(param_config.device)
        cnn12_test_acc_tsr = torch.empty(1, 0).to(param_config.device)
        cnn21_train_acc_tsr = torch.empty(1, 0).to(param_config.device)
        cnn21_test_acc_tsr = torch.empty(1, 0).to(param_config.device)
        cnn22_train_acc_tsr = torch.empty(1, 0).to(param_config.device)
        cnn22_test_acc_tsr = torch.empty(1, 0).to(param_config.device)
        ...
        for kfold_idx in torch.arange(param_config.n_kfolds):
            ...
            cnn11_valid_acc, cnn12_valid_acc, cnn21_valid_acc, cnn22_valid_acc = \
                bnn_cmp_mthds(param_config, train_data, test_data, kfold_idx)
        ...
        data_save_file = f"{data_save_dir}/acc_bnn_{param_config.inference}_{param_config.dataset_folder}" \
