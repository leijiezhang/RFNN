...
from models.model_run_f1 import run_fpn_model, run_fnn_fnn_mlp, run_fnn_fnn_fc
import torch
import os
...
def run_fpn(param_config: ParamConfig):
    ...
    for i in torch.arange(len(param_config.dataset_list)):
        ...
        for kfold_idx in torch.arange(param_config.n_kfolds):
            ...
            fpn_train_f1, fpn_valid_f1 = run_fpn_model(param_config, train_data, test_data, kfold_idx)
        ...
        data_save_file = f"{data_save_dir}/f1_fpnl_{param_config.dataset_folder}" \
...
def run_fnn_fc(param_config: ParamConfig):
    param_config.log.info(f"dataset : {param_config.dataset_folder}")
    ...
    for i in torch.arange(len(param_config.dataset_list)):
        ...
        for kfold_idx in torch.arange(param_config.n_kfolds):
            ...
            fpn_train_acc, fpn_valid_acc = \
                run_fnn_fnn_fc(param_config, train_data, test_data, kfold_idx)
...
def run_fnn_mlp(param_config: ParamConfig):
    param_config.log.info(f"dataset : {param_config.dataset_folder}")
    param_config.log.info(f"rule number : {param_config.n_rules}")
    param_config.log.info(f"rule number in FS layer : {param_config.n_rules_fs}")
    param_config.log.info(f"batch_size : {param_config.n_batch}")
