...
class BNN(PyroModule):
    ...
    def evaluate_f1(self, test_loader, device, n_cls, n_samples=10, seeds_list=None):
        self.device = device
        self.basenet.device = device
        self.to(device)
        self.basenet.to(device)
        ...
        outputs_train = torch.empty(0, n_cls).to(self.device)

        gnd_train = torch.empty(0, 1).to(self.device)
        with torch.no_grad():
            correct_predictions = 0.0
            for x_batch, y_batch in test_loader:
                x_batch = x_batch.to(device)
                outputs = self.forward(x_batch, n_samples=n_samples, seeds=bnn_seeds)
                outputs_train = torch.cat((outputs_train, outputs), 0)
                gnd_train = torch.cat((gnd_train, y_batch), 0)
            _, predicted_train = torch.max(outputs_train, 1)
            f1 = F1Score(num_classes=n_cls).to(self.device)
            f1_value = f1(predicted_train, gnd_train.long().squeeze())
            # print("Accuracy: %.2f%%" % (accuracy))
            return f1_value