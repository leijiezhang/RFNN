...
from models.model_run_f1 import dgp_cmp_mthds
import torch
import os
...
def run_dgp(param_config: ParamConfig):
    ...
    for i in torch.arange(len(param_config.dataset_list)):
        ...
        # dgp saving tensors
        # dgp121_train_f1_tsr = torch.empty(param_config.n_epoch, 0).to(param_config.device)
        # dgp121_test_f1_tsr = torch.zeros(param_config.n_epoch, 0).to(param_config.device)
        # dgp421_train_f1_tsr = torch.empty(param_config.n_epoch, 0).to(param_config.device)
        # dgp421_test_f1_tsr = torch.zeros(param_config.n_epoch, 0).to(param_config.device)
        dgp21_train_f1_tsr = torch.empty(param_config.n_epoch_dgp, 0).to(param_config.device)
        dgp21_test_f1_tsr = torch.zeros(param_config.n_epoch_dgp, 0).to(param_config.device)
        # dgp12421_train_f1_tsr = torch.empty(param_config.n_epoch, 0).to(param_config.device)
        # dgp12421_test_f1_tsr = torch.zeros(param_config.n_epoch, 0).to(param_config.device)
        # dgp212_train_f1_tsr = torch.empty(param_config.n_epoch, 0).to(param_config.device)
        # dgp212_test_f1_tsr = torch.zeros(param_config.n_epoch, 0).to(param_config.device)
        # dgp42124_train_f1_tsr = torch.empty(param_config.n_epoch, 0).to(param_config.device)
        # dgp42124_test_f1_tsr = torch.zeros(param_config.n_epoch, 0).to(param_config.device)
        ...
        for kfold_idx in torch.arange(param_config.n_kfolds):
            ...
            # dgp121_train_f1, dgp421_train_f1, dgp21_train_f1, dgp12421_train_f1, dgp212_train_f1, dgp42124_train_f1, \
            ...
            dgp21_train_f1, dgp21_valid_f1 = dgp_cmp_mthds(param_config, train_data, test_data, kfold_idx)

            # dgp saving tensors
            # dgp121_test_f1_tsr = torch.cat([dgp121_test_f1_tsr, dgp121_valid_f1], 1)
            # dgp121_train_f1_tsr = torch.cat([dgp121_train_f1_tsr, dgp121_train_f1], 1)
            # dgp421_test_f1_tsr = torch.cat([dgp421_test_f1_tsr, dgp421_valid_f1], 1)
            # dgp421_train_f1_tsr = torch.cat([dgp421_train_f1_tsr, dgp421_train_f1], 1)
            dgp21_test_f1_tsr = torch.cat([dgp21_test_f1_tsr, dgp21_valid_f1], 1)
            dgp21_train_f1_tsr = torch.cat([dgp21_train_f1_tsr, dgp21_train_f1], 1)
            # dgp12421_test_f1_tsr = torch.cat([dgp12421_test_f1_tsr, dgp12421_valid_f1], 1)
            # dgp12421_train_f1_tsr = torch.cat([dgp12421_train_f1_tsr, dgp12421_train_f1], 1)
            # dgp212_test_f1_tsr = torch.cat([dgp212_test_f1_tsr, dgp212_valid_f1], 1)
            # dgp212_train_f1_tsr = torch.cat([dgp212_train_f1_tsr, dgp212_train_f1], 1)
            # dgp42124_test_f1_tsr = torch.cat([dgp42124_test_f1_tsr, dgp42124_valid_f1], 1)
            # dgp42124_train_f1_tsr = torch.cat([dgp42124_train_f1_tsr, dgp42124_train_f1], 1)
        ...
        save_dict = dict()
        # save dgp results
        # save_dict["dgp121_test_f1_tsr"] = dgp121_test_f1_tsr.cpu().numpy()
        # save_dict["dgp121_train_f1_tsr"] = dgp121_train_f1_tsr.cpu().numpy()
        # save_dict["dgp421_test_f1_tsr"] = dgp421_test_f1_tsr.cpu().numpy()
        # save_dict["dgp421_train_f1_tsr"] = dgp421_train_f1_tsr.cpu().numpy()
        save_dict["dgp21_test_f1_tsr"] = dgp21_test_f1_tsr.cpu().numpy()
        save_dict["dgp21_train_f1_tsr"] = dgp21_train_f1_tsr.cpu().numpy()
        # save_dict["dgp12421_test_f1_tsr"] = dgp12421_test_f1_tsr.cpu().numpy()
        # save_dict["dgp12421_train_f1_tsr"] = dgp12421_train_f1_tsr.cpu().numpy()
        # save_dict["dgp212_test_f1_tsr"] = dgp212_test_f1_tsr.cpu().numpy()
        # save_dict["dgp212_train_f1_tsr"] = dgp212_train_f1_tsr.cpu().numpy()
        # save_dict["dgp42124_test_f1_tsr"] = dgp42124_test_f1_tsr.cpu().numpy()
        # save_dict["dgp42124_train_f1_tsr"] = dgp42124_train_f1_tsr.cpu().numpy()
        ...
        data_save_file = f"{data_save_dir}/f1_dgp_{param_config.dataset_folder}" \
