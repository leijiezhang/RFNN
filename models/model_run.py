...
# from models.dgp_model1 import *
# from models.dgp_model3 import *
...
#     n_cls = dgp_model.n_cls
#     num_data = train_loader.dataset.x.shape[0]
#     optimizer = torch.optim.Adam(dgp_model.parameters(), lr=param_config.lr)
#     elbo = DeepApproximateMLL(VariationalELBO(dgp_model.likelihood,
#                                               dgp_model,
#                                               num_data=num_data))
#     # loss_fn = nn.CrossEntropyLoss()
#     epochs = param_config.n_epoch
...
#                 # data = data.cuda()
#                 # labels = labels.cuda()
#                 # y_one_hot = torch.zeros(data.shape[0], n_cls).to(param_config.device).scatter_(1, labels.long(), 1)
#                 # y = y_one_hot.view(-1)
#                 outputs = dgp_model(data)
#                 loss = -elbo(outputs, labels.squeeze().long())
#                 # loss = -mll(outputs, labels)
#                 # loss = -mll(outputs, y_one_hot.long())
#                 optimizer.zero_grad()
#                 loss.backward()
#                 optimizer.step()
#
...
#                 # data = data.cuda()
#                 # labels = labels.cuda()
#                 outputs_temp = dgp_model.likelihood(dgp_model(data))
#                 pred_tmp = outputs_temp.probs.mean(0).argmax(-1)
#                 outputs_train = torch.cat((outputs_train, pred_tmp.unsqueeze(1)), 0)
#                 gnd_train = torch.cat((gnd_train, labels), 0)
#             # _, predicted_train = torch.max(outputs_train, 1)
#             correct_train_num = (outputs_train.squeeze() == gnd_train.squeeze()).squeeze().sum()
#             acc_train = correct_train_num / gnd_train.shape[0]
...
#
#         param_config.log.info(
#             f"{model_name} epoch : {epoch + 1}, train acc : {dgp_train_acc[-1, 0]}, test acc : {dgp_valid_acc[-1, 0]}")
#
#     param_config.log.info(f":======================={model_name} finished===========================")
...
#
#
# def dgp_cls1(dgp_model: DeepGP, param_config: ParamConfig, train_loader: DataLoader, valid_loader: DataLoader, model_name):
#     """
#         todo: this is the method for fuzzy Neuron network using kmeans
#         :param dnn_model: mlp model
#         :param param_config: config information
...
#     n_cls = dgp_model.n_cls
#     optimizer = torch.optim.Adam(dgp_model.parameters(), lr=param_config.lr)
#     mll = DeepApproximateMLL(VariationalELBO(dgp_model.likelihood, dgp_model, train_loader.dataset.x.shape[-2]))
#     # loss_fn = nn.CrossEntropyLoss()
#     epochs = param_config.n_epoch
#     n_para = sum(param.numel() for param in dgp_model.parameters())
#     param_config.log.info(f'# generator parameters: {n_para}')
...
#                 y = y_one_hot.view(-1)
#                 outputs = dgp_model(data)
#                 # loss = mll(outputs, labels.squeeze().long())
#                 # loss = -mll(outputs, labels)
#                 loss = -mll(outputs, y_one_hot.long())
#                 optimizer.zero_grad()
#                 loss.backward()
...
#
#         dgp_model.eval()
#         predictive_means, predictive_variances, test_lls = dgp_model.predict(valid_loader.dataset.x)
#         outputs_train = torch.empty(0, n_cls).to(param_config.device)
#         outputs_val = torch.empty(0, n_cls).to(param_config.device)
#
#         gnd_train = torch.empty(0, 1).to(param_config.device)
...
#
#
# def dgp_cls3(dgp_model: DeepGP, param_config: ParamConfig, train_loader: DataLoader, valid_loader: DataLoader, model_name):
#     """
#         todo: this is the method for fuzzy Neuron network using kmeans
#         :param dnn_model: mlp model
#         :param param_config: config information
...
def dgp_cmp_mthds1(param_config: ParamConfig, train_data: Dataset, test_data: Dataset, current_k):
    """
...
def dgp_cmp_mthds(param_config: ParamConfig, train_data: Dataset, test_data: Dataset, current_k):
    ...
    # ============different types of mlp models=========
    dgp_model: nn.Module = Normal_DeepGP(train_data.n_fea, n_cls)
    dgp121_train_acc, dgp121_valid_acc = dgp_cls(dgp_model, param_config, train_loader, valid_loader, "dgp121")
    dgp_model: nn.Module = DeepGP121(train_data.n_fea, n_cls, param_config.device)
    dgp121_train_acc, dgp121_valid_acc = dgp_cls1(dgp_model, param_config, train_loader, valid_loader, "dgp121")
...
def dgp_cmp_mthds3(param_config: ParamConfig, train_data: Dataset, test_data: Dataset, current_k):
    """
    todo: this is the method for deep gaussian process
    :param param_config:
    :param train_data: training dataset
    ...
    Xu = torch.from_numpy(kmeans2(X.cpu().numpy(), 100)[0]).to(param_config.device)
    ...
    mean_fn.cuda()
    dgp_model: nn.Module = DeepGP3(X, y, n_fea, n_cls, Xu, mean_fn)
    dgp121_train_acc, dgp121_valid_acc = dgp_cls3(dgp_model, param_config, train_loader, valid_loader, "dgp121")
    dgp_model: nn.Module = DeepGP121(train_data.n_fea, n_cls, param_config.device)
    dgp121_train_acc, dgp121_valid_acc = dgp_cls1(dgp_model, param_config, train_loader, valid_loader, "dgp121")
