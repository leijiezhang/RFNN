...
from models.model_run_f1 import bnn_cmp_mthds
import torch
import os
...
def run_bnn(param_config: ParamConfig):
    ...
    for i in torch.arange(len(param_config.dataset_list)):
        ...
        # mlp saving tensors
        mlp121_train_f1_tsr = torch.empty(1, 0).to(param_config.device)
        mlp121_test_f1_tsr = torch.empty(1, 0).to(param_config.device)
        mlp421_train_f1_tsr = torch.empty(1, 0).to(param_config.device)
        mlp421_test_f1_tsr = torch.empty(1, 0).to(param_config.device)
        mlp21_train_f1_tsr = torch.empty(1, 0).to(param_config.device)
        mlp21_test_f1_tsr = torch.empty(1, 0).to(param_config.device)
        mlp12421_train_f1_tsr = torch.empty(1, 0).to(param_config.device)
        mlp12421_test_f1_tsr = torch.empty(1, 0).to(param_config.device)
        mlp212_train_f1_tsr = torch.empty(1, 0).to(param_config.device)
        mlp212_test_f1_tsr = torch.empty(1, 0).to(param_config.device)
        mlp42124_train_f1_tsr = torch.empty(1, 0).to(param_config.device)
        mlp42124_test_f1_tsr = torch.empty(1, 0).to(param_config.device)

        # 1d-cnn saving tensors
        cnn11_train_f1_tsr = torch.empty(1, 0).to(param_config.device)
        cnn11_test_f1_tsr = torch.empty(1, 0).to(param_config.device)
        cnn12_train_f1_tsr = torch.empty(1, 0).to(param_config.device)
        cnn12_test_f1_tsr = torch.empty(1, 0).to(param_config.device)
        cnn21_train_f1_tsr = torch.empty(1, 0).to(param_config.device)
        cnn21_test_f1_tsr = torch.empty(1, 0).to(param_config.device)
        cnn22_train_f1_tsr = torch.empty(1, 0).to(param_config.device)
        cnn22_test_f1_tsr = torch.empty(1, 0).to(param_config.device)
        ...
        data_save_file = f"{data_save_dir}/f1_bnn_{param_config.inference}_{param_config.dataset_folder}" \
