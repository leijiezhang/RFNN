...
from models.model_run_f1 import lgb_cmp_mthds
import torch
import os
import scipy.io as io


def run_lgb(param_config: ParamConfig):
    """
    ...
    param_config.log.info(f"lr : {param_config.lr}")
    ...
    for i in torch.arange(len(param_config.dataset_list)):
        ...
        # lgb saving tensors
        # lgb121_train_f1_tsr = torch.empty(param_config.n_epoch, 0).to(param_config.device)
        # lgb121_test_f1_tsr = torch.zeros(param_config.n_epoch, 0).to(param_config.device)
        # lgb421_train_f1_tsr = torch.empty(param_config.n_epoch, 0).to(param_config.device)
        # lgb421_test_f1_tsr = torch.zeros(param_config.n_epoch, 0).to(param_config.device)
        lgb_train_f1_tsr = torch.empty(1, 0).to(param_config.device)
        lgb_test_f1_tsr = torch.zeros(1, 0).to(param_config.device)
        # lgb12421_train_f1_tsr = torch.empty(param_config.n_epoch, 0).to(param_config.device)
        # lgb12421_test_f1_tsr = torch.zeros(param_config.n_epoch, 0).to(param_config.device)
        # lgb2_train_f1_tsr = torch.empty(param_config.n_epoch, 0).to(param_config.device)
        # lgb2_test_f1_tsr = torch.zeros(param_config.n_epoch, 0).to(param_config.device)
        # lgb42124_train_f1_tsr = torch.empty(param_config.n_epoch, 0).to(param_config.device)
        # lgb42124_test_f1_tsr = torch.zeros(param_config.n_epoch, 0).to(param_config.device)
        ...
        for kfold_idx in torch.arange(param_config.n_kfolds):
            ...
            # lgb121_train_f1, lgb421_train_f1, lgb_train_f1, lgb12421_train_f1, lgb2_train_f1, lgb42124_train_f1, \
            # lgb121_valid_f1, lgb421_valid_f1, lgb_valid_f1, lgb12421_valid_f1, lgb2_valid_f1, lgb42124_valid_f1 \
            #     = lgb_cmp_mthds(param_config, train_data, test_data, kfold_idx)
            lgb_train_f1, lgb_valid_f1 = lgb_cmp_mthds(param_config, train_data, test_data, kfold_idx)

            # lgb saving tensors
            # lgb121_test_f1_tsr = torch.cat([lgb121_test_f1_tsr, lgb121_valid_f1], 1)
            # lgb121_train_f1_tsr = torch.cat([lgb121_train_f1_tsr, lgb121_train_f1], 1)
            # lgb421_test_f1_tsr = torch.cat([lgb421_test_f1_tsr, lgb421_valid_f1], 1)
            # lgb421_train_f1_tsr = torch.cat([lgb421_train_f1_tsr, lgb421_train_f1], 1)
            lgb_test_f1_tsr = torch.cat([lgb_test_f1_tsr, lgb_valid_f1.unsqueeze(0).unsqueeze(1)], 1)
            lgb_train_f1_tsr = torch.cat([lgb_train_f1_tsr, lgb_train_f1.unsqueeze(0).unsqueeze(1)], 1)
            # lgb12421_test_f1_tsr = torch.cat([lgb12421_test_f1_tsr, lgb12421_valid_f1], 1)
            # lgb12421_train_f1_tsr = torch.cat([lgb12421_train_f1_tsr, lgb12421_train_f1], 1)
            # lgb2_test_f1_tsr = torch.cat([lgb2_test_f1_tsr, lgb2_valid_f1], 1)
            # lgb2_train_f1_tsr = torch.cat([lgb2_train_f1_tsr, lgb2_train_f1], 1)
            # lgb42124_test_f1_tsr = torch.cat([lgb42124_test_f1_tsr, lgb42124_valid_f1], 1)
            # lgb42124_train_f1_tsr = torch.cat([lgb42124_train_f1_tsr, lgb42124_train_f1], 1)
        ...
        save_dict = dict()
        # save lgb results
        # save_dict["lgb121_test_f1_tsr"] = lgb121_test_f1_tsr.cpu().numpy()
        # save_dict["lgb121_train_f1_tsr"] = lgb121_train_f1_tsr.cpu().numpy()
        # save_dict["lgb421_test_f1_tsr"] = lgb421_test_f1_tsr.cpu().numpy()
        # save_dict["lgb421_train_f1_tsr"] = lgb421_train_f1_tsr.cpu().numpy()
        save_dict["lgb_test_f1_tsr"] = lgb_test_f1_tsr.cpu().numpy()
        save_dict["lgb_train_f1_tsr"] = lgb_train_f1_tsr.cpu().numpy()
        # save_dict["lgb12421_test_f1_tsr"] = lgb12421_test_f1_tsr.cpu().numpy()
        # save_dict["lgb12421_train_f1_tsr"] = lgb12421_train_f1_tsr.cpu().numpy()
        # save_dict["lgb2_test_f1_tsr"] = lgb2_test_f1_tsr.cpu().numpy()
        # save_dict["lgb2_train_f1_tsr"] = lgb2_train_f1_tsr.cpu().numpy()
        # save_dict["lgb42124_test_f1_tsr"] = lgb42124_test_f1_tsr.cpu().numpy()
        # save_dict["lgb42124_train_f1_tsr"] = lgb42124_train_f1_tsr.cpu().numpy()
        ...
        data_save_file = f"{data_save_dir}/f1_lgb_{param_config.dataset_folder}" \
