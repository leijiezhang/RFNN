...
from models.model_run_f1 import fnn_cmp
import torch
import os
...
def run_fnn(param_config: ParamConfig):
    ...
    for i in torch.arange(len(param_config.dataset_list)):
        ...
        fnn_train_f1_tsr = torch.empty(rule_list.shape[0], 0).to(param_config.device)
        fnn_test_f1_tsr = torch.zeros(rule_list.shape[0], 0).to(param_config.device)
        ...
        for kfold_idx in torch.arange(param_config.n_kfolds):
            ...
            fnn_train_f1, fnn_valid_f1 = fnn_cmp(param_config, train_data, test_data, rule_list, kfold_idx)

            fnn_test_f1_tsr = torch.cat([fnn_test_f1_tsr, fnn_valid_f1], 1)
            fnn_train_f1_tsr = torch.cat([fnn_train_f1_tsr, fnn_train_f1], 1)
        ...
        save_dict = dict()
        # save fnn results
        save_dict["fnn_test_f1_tsr"] = fnn_test_f1_tsr.cpu().numpy()
        save_dict["fnn_train_f1_tsr"] = fnn_train_f1_tsr.cpu().numpy()

        data_save_file = f"{data_save_dir}/f1_fnn_{param_config.dataset_folder}" \
