...
class Neuron(object):
    ...
    def predict(self, data: Dataset):
        ...
        self.__rules.update_rules(data.fea, self.__rules.center_list)
        h, _ = self.__h_computer.comute_h(data.fea, self.__rules)
        n_rule = h.shape[0]
        n_smpl = h.shape[1]
        n_fea = h.shape[2]
        ...
        # calculate Y hat
        y_hat = h_cal.mm(self.__rules.consequent_list.reshape(data.gnd.shape[1],
                                                              n_rule * n_fea).t())
        return y_hat
...
class NeuronC(Neuron):
    ...
    def forward(self, **kwargs):
        ...
        rules.fit(data.fea, n_rules)
        h, _ = h_computer.comute_h(data.fea, rules)
        # run FNN solver for given rule number
        fnn_solver.h = h
        fnn_solver.y = data.gnd.double()
...
#                                                           n_agents, rules)
#         n_fea = d_data[1].fea.shape[1]
#         n_output = d_data[1].gnd.shape[1]
#         h_all_agent = []
...
#             rules.update_rules(d_data[int(i)].fea, center_optimal)
#             h_per_agent, _ = h_computer.comute_h(d_data[int(i)].fea, rules)
#             h_all_agent.append(h_per_agent)
#
#             fnn_solver.h = h_per_agent
#             fnn_solver.gnd = d_data[int(i)].gnd.double()
#             fnn_solver.para_mu = para_mu
