...
from models.model_run_f1 import run_cmp_mthds
import torch
import os
...
def run_dnn(param_config: ParamConfig):
    ...
    for i in torch.arange(len(param_config.dataset_list)):
        ...
        fpn_train_f1_tsr = torch.empty(param_config.n_epoch, 0).to(param_config.device)
        fpn_test_f1_tsr = torch.zeros(param_config.n_epoch, 0).to(param_config.device)
        # mlp saving tensors
        ...
        svm_train_f1_tsr = torch.empty(1, 0).to(param_config.device)
        svm_test_f1_tsr = torch.zeros(1, 0).to(param_config.device)
        ...
        for kfold_idx in torch.arange(param_config.n_kfolds):
            ...
            fpn_test_f1_tsr = torch.cat([fpn_test_f1_tsr, fpn_valid_f1], 1)
            fpn_train_f1_tsr = torch.cat([fpn_train_f1_tsr, fpn_train_f1], 1)
            # mlp saving tensors
            ...
            # svm saving tensors
            svm_test_f1_tsr = torch.cat([svm_test_f1_tsr, svm_test_f1], 1)
            svm_train_f1_tsr = torch.cat([svm_train_f1_tsr, svm_train_f1], 1)
        data_save_dir = f"./results/{param_config.dataset_folder}"

        if not os.path.exists(data_save_dir):
            os.makedirs(data_save_dir)
        save_dict = dict()
        save_dict["fpn_test_f1_tsr"] = fpn_test_f1_tsr.cpu().numpy()
        save_dict["fpn_train_f1_tsr"] = fpn_train_f1_tsr.cpu().numpy()
        # save mlp results
        ...
        save_dict["svm_test_f1_tsr"] = svm_test_f1_tsr.cpu().numpy()
        save_dict["svm_train_f1_tsr"] = svm_train_f1_tsr.cpu().numpy()

        data_save_file = f"{data_save_dir}/f1_dnn_{param_config.dataset_folder}" \
